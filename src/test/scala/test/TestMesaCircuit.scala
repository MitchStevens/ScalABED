...
/**
  ...
  */
class TestMesaCircuit extends FlatSpec {
  ...
  it must "simulate a not gate correctly" in {
    ...
    assert_signals(f(Signal(F)), Array(e, Signal(T), e, e))
    assert_signals(f(Signal(T)), Array(e, Signal(F), e, e))
  }
  ...
  it must "simulate an xor gate correctly" in {
    ...
    assert(f.size == 8)
    assert_signals(f(Signal(F, F)), Array(e, Signal(F), e, e))
    f(Signal(F, T))
    //assert_signals(f(Signal(F, T)), Array(e, Signal(T), e, e))
    println(f.state)
    assert_signals(f(Signal(T, F)), Array(e, Signal(T), e, e))
    assert_signals(f(Signal(T, T)), Array(e, Signal(F), e, e))
  }
  ...
  it must "simulate an rs latch correctly" in {
    ...
    val set   = Signal(0, 1)
    def is_on() = assert_signals(f.recieved_outputs(f.state), Array(e, Signal(F), Signal(T), e))
    def is_off()  = assert_signals(f.recieved_outputs(f.state), Array(e, Signal(T), Signal(F), e))
